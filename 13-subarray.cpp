...
int main()
{
  ...
  for (int val : nums)
  {
    ...
    maxSum = max(currentSum, maxSum); // here maximum of currentSum and maxSum will be stored in maxSum.
    if (currentSum < 0)
    { // if the currentSum is less than 0, if the value is (-negative), currentSum will be set ot 0
      currentSum = 0;
    }
